...
from models import MaintenanceTask, Bill
from schemas import MaintenanceCreate, MaintenanceUpdate, MaintenanceOut, MaintenanceCompleteBody
...
def delete_task(task_id: int, db: Session = Depends(get_db)):
    ...
    db.query(Bill).filter(
        Bill.entity_type == "maintenance_task", Bill.entity_id == task_id
    ).delete()
    db.delete(db_task)
    db.commit()
...
def complete_task(task_id: int, body: MaintenanceCompleteBody = None, db: Session = Depends(get_db)):
    ...
    bill = Bill(
        ...
        entity_id=task_id,
        bill_date=today,
        amount=body.cost if body else None,
    )
    db.add(bill)
    db.commit()
    db.refresh(db_task)
