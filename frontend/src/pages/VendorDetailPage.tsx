...
export default function VendorDetailPage() {
  ...
  if (!vendor) return <p className="text-holo-500 font-medium animate-pulse">Loading...</p>;

  return (
    <div>
      <Link to="/vendors" className="text-holo-500 hover:text-neon-400 text-sm font-medium transition-colors">&larr; Back to Vendors</Link>
      <h1 className="font-heading text-2xl text-holo-50 mt-2 mb-4">{vendor.name}</h1>
      <div className="bg-void-400 rounded-xl border border-void-200 p-6 mb-8">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <div className="text-holo-500 text-xs font-semibold uppercase tracking-wider mb-1">Service Type</div>
            <div>{vendor.service_type}</div>
          </div>
          <div>
            <div className="text-holo-500 text-xs font-semibold uppercase tracking-wider mb-1">Phone</div>
            <div>{vendor.phone || '—'}</div>
          </div>
          <div>
            <div className="text-holo-500 text-xs font-semibold uppercase tracking-wider mb-1">Email</div>
      ...
      {quotes.length === 0 ? <p className="text-holo-600 text-sm italic mb-6">No quotes.</p> : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-void-100">
              <tr>
                ...
                <th className="text-left px-5 py-3.5 text-xs font-semibold text-holo-500 uppercase tracking-wider">Project</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-void-200">
              {quotes.map(q => (
                <tr key={q.id} className="hover:bg-void-50 transition-colors">
                  ...
                  <td className="px-5 py-4">{q.project ? <Link to={`/projects/${q.project.id}`} className="text-neon-400 hover:text-neon-300 font-medium transition-colors">{q.project.name}</Link> : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <h2 className="font-heading text-lg text-holo-100 mb-2">Contracts & Warranties</h2>
      {contracts.length === 0 ? <p className="text-holo-600 text-sm italic">No contracts.</p> : (
